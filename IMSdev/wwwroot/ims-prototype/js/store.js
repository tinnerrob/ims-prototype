...
IMS.store = (function(){
  ...
  function table(name){
    const map = {
      ...
      invoices: IMS.invoices, assetAttachments: IMS.assetAttachments, healthcare: IMS.healthcare, receivings: IMS.receivings,
  ...
  const ARRAY_NAMES = [
    ...
    "dispatches", "invoices", "rentals", "vehicles", "healthcare", "receivings"
  ];
  ...
  function ensureCoreFields(){
    ...
    types.forEach(t => {
      (IMS.itemRegistry.getByType(t) || []).forEach(r => {
        ...
        if (t === "serialized"){
          const RELOC = [ ["serial_vin", "serial"], ["make", "make"], ["model", "model"], ["meter_hours", "meterHours"], ["fuel_type", "fuelType"] ];
